...
class PCL(object):
    """
    A class for embedding sentences, calculating similarities, and retriving sentences by PCL.
    """
...
if __name__=="__main__":
    ...
    similarities = pcl.similarity(example_queries, example_sentences)
    print(similarities)

    print("\n=========Naive brute force search============\n")
    pcl.build_index(example_sentences, use_faiss=False)
    results = pcl.search(example_queries)
    ...
    pcl.build_index(example_sentences, use_faiss=True)
    results = pcl.search(example_queries)
    for i, result in enumerate(results):
        print("Retrieval results for query: {}".format(example_queries[i]))
        for sentence, score in result:
